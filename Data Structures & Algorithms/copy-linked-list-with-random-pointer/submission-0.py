...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        new_sentinal = Node(0)
        new_current = new_sentinal

        current = sentinal.next
        counter = 0
        while current:
            addr_indx[hash(current)] = counter
            counter += 1
            current = current.next

        addr_indx[hash(None)] = counter


        current = sentinal.next
        while current:
            val_random_indx.append((current.val, addr_indx[hash(current.random)])) #val, random indx
            current=current.next
            
        # del addr_indx

        for val, _ in val_random_indx:
            new_nodes.append(Node(val))
